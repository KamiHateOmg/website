 ...
 */
const getSystemLogs = async (pool, filters) => {
    try {
        const { action, userId, days, limit = 100, offset = 0 } = filters;
 ...
 * Bulk key generation
 */
const generateBulkKeys = async (pool, keyData, generatedBy) => {
    try {
        const { productId, quantity, expiresInDays, description } = keyData;
        ...
        // Generate keys
        const expirationDate = expiresInDays ? new Date(Date.now() + expiresInDays * 24 * 60 * 60 * 1000) : null;
 ...
 */
const getKeyStats = async (pool, filters) => {
    try {
        const { productId, timeframe = 'all' } = filters;
 ...
 */
const performSystemCleanup = async (pool, operations, performedBy) => {
    try {
        const results = {};
        ...
        logger.info(`System cleanup performed by ${performedBy}:`, results);
 ...
 */
const exportUserData = async (pool, userId, exportedBy) => {
    try {
        // Get user data
 ...
 * System health check
 */
const getSystemHealth = async (pool) => {
    try {
