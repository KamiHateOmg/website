...
/**
 * Format user for API response (excluding sensitive data)
 */
const formatUser = (user, includeSensitive = false) => {
    const formatted = {
        id: user.id,
        email: user.email,
        role: user.role,
 ...
 */
const getUserProfile = async (pool, userId) => {
    try {
        const result = await pool.query(
            `SELECT id, email, role, email_verified, created_at, last_login, last_ip, hwid, is_active
        ...
        return formatUser(result.rows[0], true);
    } catch (error) {
 ...
 * Update user profile
 */
const updateUserProfile = async (pool, userId, updates, currentUserEmail) => {
    try {
 ...
 */
const getUserActiveSubscription = async (pool, userId) => {
    try {
 ...
 * Get user's keys
 */
const getUserKeys = async (pool, userId) => {
    try {
        const result = await pool.query(
            `SELECT k.id, k.key_code, k.redeemed_at, k.expires_at, k.is_active,
                    k.hwid_lock, k.created_at, k.redemption_ip, k.purchased_by, k.redeemed_by,
                    p.name as product_name, p.duration_days, p.price,
 ...
 */
const getUserHistory = async (pool, userId, filters = {}) => {
    try {
        const { type = 'all', days, limit = 50, offset = 0 } = filters;
        let history = [];
 ...
 * Update user HWID
 */
const updateUserHwid = async (pool, userId, hwid) => {
    try {
        ...
        if (parseInt(hwidCheck.rows[0].count) > 0) {
            throw new Error('This hardware ID is already associated with an active subscription');
        }
 ...
 */
const adminGetAllUsers = async (pool, filters = {}) => {
    try {
        const { search, role, status, limit = 50, offset = 0 } = filters;
 ...
 */
const adminUpdateUserStatus = async (pool, userId, isActive, reason, adminId, adminRole) => {
    try {
